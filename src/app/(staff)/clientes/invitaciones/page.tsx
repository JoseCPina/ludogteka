...
export default async function InvitacionesPage() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, nombre_referencia, telefono, expira_at, usada_at, cancelada_at, cliente_id, cliente_nombre, created_at, estado"
    )
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {error ? (
        ...
        <InvitacionesPanel invitaciones={(data as InvitacionFila[]) ?? []} />
      )}
    </div>
  );
}
