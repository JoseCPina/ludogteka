-- El alta por link ahora captura también la dirección del dueño.
--
-- Por qué en el alta y no después: la dirección solo se podía capturar en
-- la ficha del cliente, en "Distancia y recolección", y el resultado fue
-- 0 de 14 clientes con dirección. El cotizador de recolección lleva
-- semanas funcionando sin que nadie pueda usarlo — no por un bug, sino
-- porque el dato nunca entra. Puesto en el alta, las direcciones se
-- juntan solas conforme llegan clientes nuevos.
--
-- Cuerpo tomado de 20260910012343_fix_vinculacion_interna_tras_guardias.sql y parcheado solo en el insert: es el mismo que ya
-- corría.
