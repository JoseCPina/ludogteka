-- El trigger grande de Fase 4: TODA reserva de guardería/hotel pasa por
-- aquí sin importar por dónde entre el INSERT/UPDATE (RLS ya permite
-- escritura directa a admin/recepción, así que esto no puede vivir solo
-- en una función/RPC opcional que alguien podría saltarse — "resuelto en
-- la base, no en la pantalla", como pidió el negocio). Cuatro
-- responsabilidades, en este orden:
--
--   1. Validar que el cambio de estado sea uno de los permitidos.
--   2. Resolver y congelar tamaño/precio — solo cuando de verdad cambian
--      perro/servicio/fechas, nunca en un cambio de estado suelto, para
--      no re-cotizar por accidente algo que ya se apalabró con el
--      cliente.
--   3. Bloqueo sanitario: perro con requisito obligatorio vencido o sin
--      registro no puede reservar, salvo excepción de admin con motivo
--      (aplica también a estética, por la misma razón: el perro entra al
--      mismo local).
--   4. Cupo: día por día del rango de la estancia, cuenta cuánto hay
--      ocupado (diurno siempre; nocturno solo si es hotel) contra el
--      cupo configurado para esa fecha, protegido con un advisory lock
--      para que dos inserts concurrentes no lean el mismo cupo
--      "disponible" y ambos pasen. A diferencia del bloqueo sanitario,
--      el cupo NO tiene excepción: si físicamente no cabe, no cabe.
--
-- 2-4 solo corren cuando la estancia de verdad va a ocupar algo (no
-- nace/queda cancelada) y solo cuando el rango de fechas es nuevo o
-- cambió — pasar de "confirmada" a "en_curso" un rato después no debe
-- volver a pelear por cupo que ya se apartó al reservar, ni re-cotizar el
-- precio si el catálogo cambió mientras tanto.
...
declare
  ...
  v_activa boolean;
begin
  -- 1. Transición de estado. Un CASE explícito en vez de una tabla de
  -- transiciones: es una máquina de estados fija que casi no cambia, y
  -- esto es más fácil de leer y de ajustar a mano si el negocio cambia el
  -- flujo. Nace en reservada/confirmada/en_curso (un walk-in puede
  -- entrar directo a en_curso, sin paso intermedio).
  ...
  -- OLD es NULL en INSERT, así que estas comparaciones ya dan verdadero
  -- por sí solas ahí (is distinct from NULL); TG_OP = 'INSERT' se deja
  -- explícito solo por claridad de lectura.
  ...
  -- 4. Cupo, día por día del rango, protegido con advisory lock. Un solo
  -- lock global (no por fecha): a este volumen de negocio no hace falta
  -- afinarlo, y así es trivial de verificar que es correcto.
  if v_fechas_cambiaron and v_activa then
...
create trigger validar_estancia
before insert or update on public.estancias
for each row execute function public.validar_estancia();
